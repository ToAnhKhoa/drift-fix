...
# Tìm đường dẫn lệnh firewall-cmd
...
def check_and_enforce_ports(allowed_ports):
    ...
    try:
        # Lệnh trả về dạng: "80/tcp 443/tcp 9090/tcp"
    ...
    for port in allowed_ports:
        port_str = f"{port}/tcp" # Chuyển số 80 thành "80/tcp"
        ...
        if port_str not in current_open_ports:
            ...
            # 3. Auto-Fix: Mở port
            # Bước A: Mở ngay lập tức (Runtime)
            subprocess.run(f"{CMD_FIREWALL} --add-port={port_str}", shell=True)
            # Bước B: Ghi vào cấu hình vĩnh viễn (Permanent) - để reset máy không mất
